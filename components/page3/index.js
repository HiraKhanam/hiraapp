...
function index() {
  ...
  const handleUpdateSubmit = (e) => {
    ...
    setItems(
      items.map((item) =>
        item.id === updateId ? { ...item, message: update } : item
      )
    );
        ...
        <ul className="text-2xl font-bold w-96 ">
          ...
          {updateInput === true && (
            <form onSubmit={handleUpdateSubmit} className="mt-4">
              ...
              />
              <button className="font-semibold flex items-center justify-center mt-4 text-sm text-white text-right items-right bg-indigo-700 px-3 py-2 rounded-md w-full">
                Update
              </button>
            </form>
          )}
